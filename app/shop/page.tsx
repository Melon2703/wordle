    ...
    <main className="flex min-h-screen flex-col bg-[var(--bg)] text-[var(--text)]">
      ...
      <section className="grid flex-1 gap-4 px-4 py-6">
        {data?.products.map((product) => (
          <article
            key={product.id}
            className="rounded-3xl border border-[color:var(--tile-border)] bg-[var(--panel)] p-5 shadow-sm"
          >
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold">{product.title}</h2>
              {product.badge ? (
                <span className="rounded-full bg-[var(--key-bg)] px-3 py-1 text-xs font-semibold uppercase">
                  {product.badge}
                </span>
              ) : null}
            </div>
            ...
            <button
              type="button"
              className="mt-4 w-full rounded-xl bg-[var(--accent)] px-4 py-3 text-sm font-semibold text-white shadow-sm"
            >
              Получить
            </button>
          </article>
        ...
        {!data ? (
          <p className="rounded-3xl border border-dashed border-[color:var(--tile-border)] bg-[var(--panel)] px-4 py-10 text-center text-sm opacity-70">
            Каталог загружается…
          </p>
        ) : null}
      </section>
