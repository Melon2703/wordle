...
export async function POST(request: Request) {
  try {
    ...
    const existingLines: GuessLine[] = existingGuesses.map(guessRowToLine);
    ...
    if (isWin) {
      try {
        await updateStreak(client, profile.profile_id);
      } catch (error) {
    ...
    if (isLost) {
      try {
        await resetStreak(client, profile.profile_id);
      } catch (error) {
